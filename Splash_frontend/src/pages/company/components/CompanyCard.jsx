...
export const CompanyCard = ({ company, isSelected, onSelect, onEdit }) => {
  ...
  return (
    <div
      ref={cardRef}
      onClick={() => onSelect()}
      className={`relative group cursor-pointer p-8 rounded-3xl border-2 transition-all duration-300 flex flex-col justify-between ${
        isSelected
          ? "border-blue-500 bg-blue-500/5 shadow-[0_0_25px_rgba(59,130,246,0.15)]"
          : "border-gray-800 bg-gray-900/40 hover:border-gray-600"
      }`}
    >
      {/* Üst Kısım: İkon, Şema ve Menü */}
      <div>
        ...
        {/* Şirket Adı */}
        <h4 className="text-2xl font-bold text-white mb-3">{company.name}</h4>

        {/* Bilgiler Alanı (Vergi No, Adres ve Açıklama) */}
        <div className="space-y-2 mb-4">
          {company.vdNo && (
            <div className="flex items-center gap-2 text-xs text-gray-400 font-mono">
              <span className="text-gray-500 font-bold uppercase">VD NO:</span>
              <span className="text-gray-300">{company.vdNo}</span>
            </div>
          )}

          {company.companyAddress && (
            <div className="flex items-start gap-2 text-xs text-gray-400">
              <span className="text-gray-500 font-bold uppercase shrink-0 pt-0.5">
                ADRES:
              </span>
      ...
      {/* Alt Kısım: Fatura Açıklaması / Notu */}
