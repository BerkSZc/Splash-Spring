...
const DEFAULT_BODY =
  "Sayın Yetkili,\n\nSeçili faturalar ekte PDF belgesi olarak bilgilerinize sunulmuştur.\n\nİyi çalışmalar dileriz.";

export default function SendBulkMailModal({
  isOpen,
  onClose,
  invoices = [],
  invoiceType,
  ...
  useEffect(() => {
    ...
    if (invoices && invoices.length > 0) {
      setSubject(`${invoices.length} Adet Faturanız`);
    }
  ...
  const handleSend = async (e) => {
    ...
    if (currentInvoices.length === 0) {
      toast.error("Gönderilecek fatura bulunamadı!");
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < currentInvoices.length; i++) {
        ...
        const opt = {
          margin: [8, 8, 8, 8],
          filename: `${inv.fileNo || "Fatura"}.pdf`,
        ...
        try {
          ...
          attachments.push({
            fileName: `${inv.fileNo || `Fatura_${inv.id}`}.pdf`,
      ...
      `;
      console.log("👉 [FRONTEND GİDEN EKLER]:", attachments);
      await sendMail({
        to: emails,
        subject: subject,
        body: bodyText,
  ...
  return createPortal(
    <div className="fixed modal-container inset-0 w-screen h-screen bg-black/85 backdrop-blur-md z-[999999] flex items-center justify-center p-4 select-none">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h3 className="text-xl font-bold text-white flex items-center gap-2">
              ✉️ Toplu E-Posta Gönder
            </h3>
            <p className="text-xs text-blue-400 mt-1">
              {invoices.length} adet fatura tek e-postaya eklenecektir.
            </p>
          </div>
          <button
            type="button"
              ...
              {isBusy ? (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs">
                    {isConvertingPdf
                      ? `PDF Hazırlanıyor (${convertingProgress.current}/${convertingProgress.total})...`
                      : "Gönderiliyor..."}
                  </span>
                </div>
              ) : (
                `Gönder (${invoices.length} Ek)`
              )}
            </button>
          </div>
        </form>
